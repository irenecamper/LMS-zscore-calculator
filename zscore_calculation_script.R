...
dxa_data <- readxl::read_excel(dxa_path, col_names = TRUE) %>%
  ...
  ## Compute derived body composition indices
  dplyr::mutate(
    FMI = FM / (height^2),
    FM_trunk_quotient_limb = FM_trunk / (FM_arm + FM_leg),
    LMI = LM / (height^2),
    ALMI = (LM_arm + LM_leg) / (height^2),
    BMI = weight / (height^2),
    # fitted_FMI  = FM / (height^x),
    # fitted_LMI  = LM / (height^x),
    # fitted_ALMI = (LM_arm + LM_leg) / (height^x),
    # fitted_BMI  = weight / (height^x)
  ) %>%
